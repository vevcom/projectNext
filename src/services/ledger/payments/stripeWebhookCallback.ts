...
function extractBalanceTransaction(paymentIntent: Stripe.PaymentIntent): Stripe.BalanceTransaction | null {
    ...
    if (!latestCharge || typeof latestCharge !== 'object') {
        logger.error(
            'Stripe payment intent event is missing latest charge object.' +
    ...
    if (!balanceTransaction || typeof balanceTransaction !== 'object') {
        logger.error(
            'Stripe payment intent event is missing balance transaction object.' +
 ...
 * This function will only ever be used one place.
