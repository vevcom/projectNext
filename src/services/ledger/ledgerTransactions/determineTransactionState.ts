 ...
 */
function validAmountSum({ ledgerEntries, payment, manualTransfer }: ExpandedLedgerTransaction) {
 ...
 */
function paymentComplete({ payment }: ExpandedLedgerTransaction) {
 ...
 */
function validFeesSum({ ledgerEntries, payment, manualTransfer }: ExpandedLedgerTransaction) {
    // NOTE: Since the number of entries in a transaction is very low (max two) we can
    // sum the amounts and fees in memory rather than doing a database aggregation.
