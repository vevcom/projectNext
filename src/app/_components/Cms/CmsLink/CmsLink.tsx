...
export default function CmsLink({ cmsLink, className, color = 'secondary', updateCmsLinkAction, canEdit }: PropTypes) {
    return (
        <div className={`${styles.CmsLink} ${className}`}>
            <Link href={cmsLink.url} className={`${styles.CmsLink} ${styles[color]}`}>{cmsLink.text}</Link>
            <CmsLinkEditor canEdit={canEdit} cmsLink={cmsLink} updateCmsLinkAction={updateCmsLinkAction} />
        </div>
    )
}
