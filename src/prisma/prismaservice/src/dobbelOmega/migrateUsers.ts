...
import type { enum_Users_sex as SEXVEVEN, PrismaClient as PrismaClientVeven } from '@/generated/veven'
...
export default async function migrateUsers(
    ...
    const yearIdMap = (x: number) => {
        const year = classes.find(c => c.year === x)
        if (!year) {
    ...
    await Promise.all(users.map(async user => {
        ...
        switch (yearsInProgramme) {
            ...
            case 5:
            {
                ...
                if (yearOfStudy5 < 1) {
                    manifest.error(`User ${user.id} is in 5 year programme but has year of study less than 1 - setting to 1`)
                    yearOfStudy5 = 1
                }
                if (yearOfStudy5 > 6) {
                    manifest.error(
...
function makeEmailUnique<X extends { email: string | null, id: number }>(
    ...
    return users.map(user => {
        ...
        if (!user.email) {
            manifest.error(`User ${user.id} has no email`)
        } else {
            ...
            if (duplicates.length > 1) {
                manifest.error(`User ${user.id} has duplicate email ${user.email}`)
...
function makeUsernameUnique<X extends { username: string, id: number }>(
    ...
    return users.map(user => {
        ...
        if (duplicates.length > 1) {
            manifest.error(`User ${user.id} has duplicate username ${user.username}`)
