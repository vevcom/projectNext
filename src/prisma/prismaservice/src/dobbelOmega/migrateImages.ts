...
async function fetchAllImagesAndUploadToStore<X extends {
    ...
    name: string,
    id: number
}>(images: X[]): Promise<((X & { fsLocation: string, fsLocationSmallSize: string, fsLocationMediumSize: string }) | null)[]> {
    const ret: ((X & { fsLocation: string, fsLocationSmallSize: string, fsLocationMediumSize: string }) | null)[] = []
    let imageCounter = 1
    ...
    for (const imageBatch of imageBatches) {
        await Promise.all(imageBatch.map(async image => {
            ...
            const fsLocation = await fetchImageAndUploadToStore(fsLocationDefaultOldVev)
